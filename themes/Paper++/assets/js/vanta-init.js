...
const getLocalStorage = () => {
  ...
  try {
    const helper = window.siteUtils?.getLocalStorage;
...
const cssColorToInt = (colorValue) => {
  if (typeof window !== "undefined") {
    const helper = window.siteUtils?.cssColorToInt;
    if (typeof helper === "function") {
      return helper(colorValue);
    }
  }
...
const getThemeColor = (variableName) => {
  if (typeof window !== "undefined") {
    const helper = window.siteUtils?.getThemeColor;
    if (typeof helper === "function") {
      return helper(variableName);
    }
  }
...
const isMobileDevice = () => {
  if (typeof window !== "undefined") {
    const helper = window.siteUtils?.isMobileDevice;
    if (typeof helper === "function") {
      return helper();
    }
...
// Helper access is provided by `nav-modules.js` via `window.siteUtils`.
